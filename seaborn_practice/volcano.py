...
def parse_args():
    ...
    parser.add_argument("-n",required=False,default=10)   #点亮上调或下调前多少的基因
...
def add_text(ax,df,y_axis):
    if len(df) > 0:
        ...
        for n,(x,y,gene) in enumerate(zip(df["log2FC"],df["padj2"],df.index)):
            ax.text(x, y, n+1, ha='center', va='center', fontsize=10)
...
def draw_volcano(csv_file,padj,fc,num):
    ...
    expression_matrix["size"] = np.where(expression_matrix["sig"],6,3)
    up_df_10 = expression_matrix[expression_matrix["type"] == "up"].sort_values(by="log2FC",ascending=False)[:10]
    down_df_10 = expression_matrix[expression_matrix["type"] == "down"].sort_values(by="log2FC",ascending=True)[:10]
    xlim = max(max(up_df_10["log2FC"].abs()),max(down_df_10["log2FC"].abs()))*1.1
    ylim = max(expression_matrix["padj2"])*1.1
    cmap = ["lightgrey","salmon","mediumpurple"]
    ...
    add_text(ax,up_df_10,1)
    add_text(ax,down_df_10,0.5)
    f.savefig('volcano.png', dpi=100, bbox_inches='tight')
